...
class SubscriberUI:
    ...
    def on_message_received(self, client, userdata, msg):
        ...
        try:
            ...
            self.text_message.insert(tk.END, f"{msg.topic}: {message}\n")
            self.text_message.configure(state='disabled')
        except UnicodeDecodeError:
            messagebox.showerror("Error", "Failed to decode message payload")
        except ValueError as e:
            messagebox.showerror("Error", str(e))
        except Exception as e:
            messagebox.showerror("Error", "Failed to handle received message: " + str(e))
